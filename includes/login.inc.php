...
if (isset($_POST['login-submit'])) {
  ...
  if (empty($mailuid) || empty($password)) {
    header("Location: ../error.php?error=emptyfields&mailuid=".$mailuid);
    exit();
  }
  else {
            echo "help!";
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
      ...
      if ($row = mysqli_fetch_assoc($result)) {
        ...
        if ($pwdCheck == false) {             
          ...
          header("Location: ../error.php?error=wrongpwd");
          exit();
        }
        ...
        header("Location: ../welcome.php?login=wronguidpwd");
        exit();
      }
    }
  }
